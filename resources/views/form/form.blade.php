...
@section('main')
    <h1 class="flex justify-center">Pagamentos</h1>
    <br>
    <div class="flex justify-center items-center">
        <div class="bg-white shadow-lg rounded-lg p-6 max-w-xs w-full">
                <form class="max-w-sm mx-auto" method="POST" action="{{ route('enviar') }}">
                    @csrf
                    <label for="card-name-input" class="sr-only">Nome do cartoo:</label>
                    <div class="relative">
                        <input type="text" name="card-name-input" id="card-name-input" class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500
                          focus:border-blue-500 block w-full pe-10 p-2.5  dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white
                          dark:focus:ring-blue-500 dark:focus:border-blue-500 mb-4"
                               placeholder="John Travolta" required/>
                    </div>
                    <label for="card-number-input" class="sr-only">Card number:</label>
                    <div class="relative">
                        <input type="text" name="card-number-input" id="card-number-input" class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500
                          focus:border-blue-500 block w-full pe-10 p-2.5  dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white
                    ...
                    <div class="grid grid-cols-3 gap-4 my-4">
                        <div class="relative max-w-sm col-span-2">
                            ...
                            <input datepicker datepicker-format="mm/yy" name="card-expiration-input" id="card-expiration-input" type="text"
                                   class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block
                                    w-full ps-10 p-2.5 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-blue-500
                                     dark:focus:border-blue-500" placeholder="12/23" required value="{{ old('card-expiration-input') }}"/>
                        </div>
                        <div class="col-span-1">
                            <label for="cvv-card" class="sr-only">Card CVV code:</label>
                            <input type="number" name="cvv-card" id="cvv-card" aria-describedby="helper-text-explanation"
                                   class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block
                                    w-full p-2.5 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-blue-500
                                     dark:focus:border-blue-500" placeholder="CVV" required value="{{ old('cvv-card') }}"/>
                        </div>
                    </div>
